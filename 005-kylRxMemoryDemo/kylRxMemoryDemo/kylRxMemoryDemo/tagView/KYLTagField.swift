...
public enum KYLTagAcceptOption {
    case `return`
    case comma
    case space
}
...
open class KYLTagsField: UIScrollView {
    ...
    /// Max number of lines of tags can display in KYLTagsField before its contents become scrollable. Default value is 0, which means KYLTagsField always resize to fit all tags.
    ...
    /// Whether or not the KYLTagsField should become scrollable
    ...
    open var acceptTagOption: KYLTagAcceptOption = .return
     ...
     */
    open var onVerifyTag: ((KYLTagsField, _ text: String) -> Bool)?
     ...
     */
    open var onDidChangeHeightTo: ((KYLTagsField, _ height: CGFloat) -> Void)?
    ...
    fileprivate var estimatedInitialMaxLayoutWidth: CGFloat {
        ...
        // This is necessary because, while using the KYLTagsField in a `UITableViewCell` with a dynamic height, the `intrinsicContentSize` is called first than the `layoutSubviews`, which leads to an unknown view width when AutoLayout is being used.
    ...
    open func addTags(_ tags: [KYLTag]) {
        tags.forEach { addTag($0) }
    }
    ...
    open func addTag(_ tag: KYLTag) {
        ...
        let tagView = KYLTagView(tag: tag)
    ...
    open func removeTag(_ tag: String) {
        removeTag(KYLTag(tag))
    }
    
    open func removeTag(_ tag: KYLTag) {
    ...
    @discardableResult
    open func tokenizeTextFieldText() -> KYLTag? {
    ...
    open func selectTagView(_ tagView: KYLTagView, animated: Bool = false) {
        if self.readOnly {
            return
        }
...
extension KYLTagsField {
    ...
    fileprivate func enumerateItemRects(layoutWidth: CGFloat, using closure: (_ tagView: KYLTagView?, _ tagRect: CGRect?, _ textFieldRect: CGRect?) -> Void) {
        if layoutWidth == 0 {
            return
        }
    ...
    fileprivate func repositionViews() {
        ...
        enumerateItemRects(layoutWidth: self.bounds.width) { (tagView: KYLTagView?, tagRect: CGRect?, textFieldRect: CGRect?) in
...
extension KYLTagsField: UITextFieldDelegate {
...
public func == (lhs: UITextField, rhs: KYLTagsField) -> Bool {
