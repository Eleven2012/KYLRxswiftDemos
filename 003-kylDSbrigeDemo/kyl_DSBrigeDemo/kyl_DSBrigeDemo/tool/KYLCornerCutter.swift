...
extension UIView {
    ...
    func kyl_roundCorner(cornerRadii: CGFloat)  {
        
        self.kyl_roundCorner(direction: UIRectCorner.allCorners, cornerRadii: cornerRadii, borderWidth: 0, borderColor: UIColor.clear, backgroundColor: UIColor.clear)
    }
    
    func kyl_roundCorner(direction: UIRectCorner, cornerRadii: CGFloat, borderWidth: CGFloat, borderColor: UIColor, backgroundColor: UIColor)  {
        if self.classForCoder == UIImageView.classForCoder() {
            KYLCornerCutter.cuttingImageView(imageView: self as! UIImageView, direction: direction, cornerRadii: cornerRadii, borderWidth: borderWidth, borderColor: borderColor, backgroundColor: backgroundColor)
        }else{
            var viewBackColor = backgroundColor
            /// 防止大家设置相应的背景色 - 这里进行转移
            if (self.backgroundColor != nil) {
                viewBackColor = self.backgroundColor!
                self.backgroundColor = nil
            }
            KYLCornerCutter.cuttingView(view: self, direction: direction, cornerRadii: cornerRadii, borderWidth: borderWidth, borderColor: borderColor, backgroundColor: viewBackColor)
        }
    }
    
}

public class KYLCornerCutter: NSObject {
